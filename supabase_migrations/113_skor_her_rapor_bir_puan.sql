-- 113: skor_liderlik — aynı firma+gün+teknisyen DISTINCT'ini kaldır.
-- Sebep: bir firmanın birden çok lokasyonu olabiliyor (şube, farklı sistemler);
-- aynı gün farklı raporlar açıldıysa hepsi ayrı iş olarak sayılmalı.
-- Migration 105 ile birebir aynı, tek fark: 'teklestirilmis' CTE'sinde DISTINCT yok.
...
begin
  ...
  -- Her satır 1 puan — DISTINCT YOK (aynı firma farklı lokasyonlar sayılsın)
  temiz as (
