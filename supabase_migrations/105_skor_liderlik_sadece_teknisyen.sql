-- 105: skor_liderlik — Oğuz/Ali gibi admin'ler leaderboard'da görünmesin.
-- 089'daki tüm mantık aynı (bakım hariç, boş ariza_kodu hariç, distinct per
-- day+firm, soyad match, foto/unvan/telefon join) — sadece kullanici_norm CTE'sinde
-- rol='admin' hariç tutulur. Böylece admin'ler ne foto match olur ne de "matched"
-- kalırsa filtre elemez; matched.k_foto NULL kalır ama sayaç yine gösterilir —
-- BU İSTENMEZ. Bu yüzden admin adı ile eşleşen tek_ad'leri gruplu CTE'sinden de
-- elemeliyiz.
...
begin
  ...
  -- Admin isimlerini normalize et — leaderboard'dan çıkarılacaklar
  admin_norm as (
  ...
  kullanici_norm as (
    ...
    from kullanicilar k
    where k.rol = 'personel'  -- sadece personel; admin zaten yukarıda elendi
  )
  select
    g.tek_ad,
    g.tek_sayi,
