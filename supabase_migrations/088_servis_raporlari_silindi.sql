-- servis_raporlari yumuşak silme flag'i
alter table servis_raporlari
  add column if not exists silindi boolean not null default false;

create index if not exists servis_raporlari_silindi_idx
  on servis_raporlari (silindi) where silindi = false;

-- Skor RPC: silinen kayıtları hariç tut
...
begin
  ...
  with birlesim as (
    ...
    where gid_tarih between baslangic and bitis
      ...
      and coalesce(ariza_kodu, '') !~* 'bakım|bakim'
      and silindi = false                     -- YENİ: silinenleri hariç
  ),
  teklestirilmis as (
