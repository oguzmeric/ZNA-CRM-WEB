...
const GORUSME_LISTE_KOLONLARI = 'id, gorusme_no, akt_no, tarih, saat, firma_adi, musteri_adi, konu, tip, durum, hazirlayan, olusturma_tarih, gorusen, takip_notu, gorusme_sonucu, muhatap_id, muhatap_ad, olusturan_id, musteri_id, irtibat_sekli, lokasyon_id'
...
export const gorusmeleriGetir = () => cached('gorusmeler:list', async () => {
  ...
  let off = 0
  while (true) {
    const { data, error } = await supabase.from('gorusmeler').select(GORUSME_LISTE_KOLONLARI).order('olusturma_tarih', { ascending: false }).range(off, off + sayfa - 1)
