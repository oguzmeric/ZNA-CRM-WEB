...
export default function SatisSozlesmeForm() {
  ...
  const yazdir = () => {
    ...
    w.document.write(`<!DOCTYPE html><html lang="tr"><head><meta charset="utf-8"><base href="${window.location.origin}/"><title>${kayit?.sozlesmeNo || 'Satış Sözleşmesi'}</title></head><body>${guvenliBelgeHtml(ssBelgeGoster(icerikHtml()))}<script>window.onload = () => setTimeout(() => window.print(), 400)</scr` + `ipt></body></html>`)
    w.document.close()
  }
